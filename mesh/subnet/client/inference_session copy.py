...
class _ServerInferenceSession:
    ...
    async def create(
        ...
        **metadata,
        ...
        stub = ServicerBase.get_stub(p2p, span.peer_id)
        inputs_queue = asyncio.Queue()
...
class InferenceSession:
    ...
    def _enter_server_sessions(self, chosen_spans: List[RemoteInfo]) -> List[_ServerInferenceSession]:
        server_sessions = []
        try:
    ...
    #     updated_spans = self._sequence_manager.make_sequence(
    #         update_end, mode="min_latency", cache_tokens_needed=self._max_length
    #     )
    #     # make_sequence() could return a longer sequence
    #     updated_spans[-1].end = min(updated_spans[-1].end, update_end)
    #     updated_sessions = self._enter_server_sessions(updated_spans)
