...
from mesh import DHT, P2P, MSGPackSerializer, PeerID
...
from mesh.proto import crypto_pb2, runtime_pb2
...
from mesh.subnet.data_structures import ModuleUID, RemoteInfo, ServerState
...
class RemoteManager:
    ...
    async def test_inference(
        ...
        try:
            if not self.ready.is_set():
                logger.debug("test_inference update")
                print("test_inference update")
            ...
            logger.debug(f"test_inference active_servers {active_servers}")
            print(f"test_inference active_servers {active_servers}")

            peer_id = random.choice(active_servers)
            logger.debug(f"test_inference peer_id {peer_id}")
            print(f"test_inference peer_id {peer_id}")

            stub = InferenceProtocol.get_server_stub(
                self.state.p2p,
                peer_id,
            )
            logger.debug(f"test_inference stub {stub}")
            print(f"test_inference stub {stub}")
                ...
                async with asyncio.Semaphore(float("inf")):
                    ...
                    async for response in response_stream:
                        for tensor_bytes in response.tensors:
                            ...
                            logger.debug(f"test_inference {tensor}")
            except Exception as e:
                logger.error(f"InferenceProtocol failed to stream from {peer_id}: {e}", exc_info=True)
                return


            logger.debug("test_inference after on_request_success")
        except Exception as e:
            logger.debug(f"test_inference Exception {e}", exc_info=True)

    async def test_inference2(
        self,
        prompt: str,
        tensor: torch.Tensor,
        hoster_peer_id: PeerID
    ):
        active_servers = [
            info.peer_id
            for info in self.state.remote_servers_infos.server_infos
        ...
        logger.debug(f"test_inference2 active_servers {active_servers}")
        print(f"test_inference2 active_servers {active_servers}")

        hoster_stub = InferenceProtocol.get_server_stub(
            self.state.p2p,
            hoster_peer_id,
        )
        logger.debug(f"test_inference2 hoster_stub {hoster_stub}")
        print(f"test_inference2 hoster_stub {hoster_stub}")
        ...
        try:
            async with asyncio.Semaphore(float("inf")):
                response_stream = await hoster_stub.rpc_inference_stream(input_stream)
                async for response in response_stream:
                    for tensor_bytes in response.tensors:
                        tensor = deserialize_torch_tensor(tensor_bytes)
                        print(tensor)
        except Exception as e:
            logger.error(f"InferenceProtocol failed to stream from {hoster_peer_id}: {e}", exc_info=True)
            return

    async def test_inference3(
        self,
        prompt: str,
        tensor: torch.Tensor,
        hoster_peer_id: PeerID
        ...
        try:
            if not self.ready.is_set():
                logger.debug("test_inference3 update")
                print("test_inference3 update")
                self.update(wait=True)
            ...
            logger.debug(f"test_inference3 active_servers {active_servers}")
            print(f"test_inference3 active_servers {active_servers}")
            logger.debug(f"test_inference3 hoster_peer_id {hoster_peer_id}")
            print(f"test_inference3 hoster_peer_id {hoster_peer_id}")

            random_peer_id = random.choice(active_servers)
            logger.debug(f"test_inference3 peer_id {random_peer_id}")
            print(f"test_inference3 peer_id {random_peer_id}")

            hoster_stub = InferenceProtocol.get_server_stub(
                self.state.p2p,
                random_peer_id,
            )
            logger.debug(f"test_inference3 hoster_stub {hoster_stub}")
            print(f"test_inference3 hoster_stub {hoster_stub}")
            ...
            try:
                ...
                logger.error(f"test_inference3 failed to stream from {hoster_peer_id}: {e}", exc_info=True)
                return
        except Exception as e:
            logger.debug(f"test_inference3 Exception {e}", exc_info=True)
    ...
    def rpc_info(self):
        ...
        if self.state.rpc_info is not None:
            logger.debug("rpc_info is not None")
        ...
        for attempt_no in itertools.count():
            ...
            try:
                ...
                # active_servers = [
                #     peer_id
                #     for peer_id, server in self.state.remote_servers_infos.server_infos
                #     if server.state == ServerState.ONLINE
                # ]
                active_servers = [
                    info.peer_id
                ...
                logger.debug(f"rpc_info active_servers {active_servers}")

                if not active_servers:
                    raise MissingServersError()

                # get info from one of the peers at random
                # TODO: get by latency
                peer_id = random.choice(active_servers)
                logger.debug(f"rpc_info peer_id {peer_id}")

                stub = ServicerBase.get_stub(self.state.p2p, peer_id)
                logger.debug(f"rpc_info stub {stub}")

                self.on_request_success(peer_id)
                logger.debug("rpc_info after on_request_success")
                break
            except Exception as e:
                logger.debug(f"rpc_info Exception {e}", exc_info=True)
        ...
        logger.debug(f"rpc_info returning rpc_info {self.state.rpc_info}")
        return self.state.rpc_info
