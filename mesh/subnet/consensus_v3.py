...
from mesh.subnet.roles.hoster_v3 import Hoster
from mesh.subnet.roles.validator_v3 import Validator
        ...
        while not self.stop.is_set() and not self._async_stop_event.is_set():
            ...
            if current_epoch != last_epoch:
                ...
                1) Validate or attest
                    1.1) Submit prompt to other nodes
                ...
                """
                2) Score nodes
                    2.1) Reveal previous epochs scores ⸺ if exists (See step 4)
                    2.2) Score hosters and validators
                        - Score ho

                At this point, every node has the scores for the subnet, although not yet revealed (or committed)
                    - Note: We wait until the hosters reveal on the same epoch to make the commit

                Who:    * All nodes do this
                When:   * 0% - 15% of epoch progress
                """
                self.validator.run(current_epoch)
                ...
                This commits the data generated from step 2
