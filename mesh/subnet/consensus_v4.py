...
from mesh.subnet.roles.hoster_v4 import Hoster
from mesh.subnet.roles.validator_v4 import Validator
...
class Consensus(threading.Thread):
    ...
    async def run_forever(self):
        ...
        while not self.stop.is_set() and not self._async_stop_event.is_set():
            ...
            if current_epoch != last_epoch:
                """
                1) Validate or attest
                    1.1) We have the scores from the previous epochs data
                    1.2) Submit prompt to other nodes

                Who:    * All nodes do this
                When:   * 0% - 15% of epoch progress
                """
                await self.run_consensus(current_epoch, current_block)

                """
                2) Reveal our previous epochs scores ⸺ if exists

                These reveals are used to score validators honesty versus their commits
                and on-chain consensus data based of attestation ratio.

                Validators don't reveal the previous epochs scores until the next epoch
                to ensure validators can't copy others nodes scores and know whether
                to attest or not

                Who:    * All nodes do this
                When:   * 0% - 15% of epoch progress
                """
                self.validator.reveal(current_epoch)

                """
                3) Run inference on prompt (See step 1.1)
                    3.1) Commit hash of inference outputs
                        When:   * 15% - 50% of epoch progress
                    3.3) Reveal commit
                        When:   * 50% - 60% of epoch progress

                Who: * Hosters only

                This can run up to 60% of the epochs progress
                """
                # Wait until 15% of epoch to start
                self.hoster.run()

                """
                4) Commit hash of all nodes scores

                We wait until after the hosters reveal deadline to score nodes
                - Hosters are scored based on the current epochs commit-reveal
                - Validators are scored based on the commit n-1 epochs ago and reveal in this epoch

                After this is complete, we have all of each nodes scores

                Who: * All nodes do this
                When:   * 60% - 100% of epoch progress
                """
    ...
    async def run_consensus(self, epoch: int, block: int):
        ...
        if self.is_validator(epoch):
            ...
            2. Store it
            """
            consensus_records = self.dht.get(consensus_key, latest=True) or {}
            ...
            while not self.stop.is_set():
                ...
                await asyncio.sleep(
                    max(0.0, epoch_data.seconds_per_epoch * 0.2 - epoch_data.seconds_elapsed)
                )
