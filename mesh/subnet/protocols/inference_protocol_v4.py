...
from mesh.proto import dht_pb2, inference_protocol_pb2
...
class InferenceProtocol(mp.context.ForkProcess, ServicerBase):
    ...
    async def call_inference_stream(
        ...
        """
        logger.info("call_inference_stream")
        ...
        try:
            async with self.rpc_semaphore:
                ...
                logger.info("call_inference_stream getting stub")
                response_stream = await self.get_stub(p2p, peer).rpc_inference_stream(input_stream)
                logger.info(f"call_inference_stream response_stream {response_stream}")
                async for response in response_stream:
                    logger.info(f"replicate_p2p response {response}")
                    for tensor_bytes in response.tensors:
                        logger.info(f"replicate_p2p tensor_bytes {tensor_bytes}")
                        tensor = deserialize_torch_tensor(tensor_bytes)
                        yield tensor
                # async for response in await self.get_stub(p2p, peer).rpc_inference_stream(input_stream):
                #     logger.info(f"replicate_p2p response {response}")
                #     for tensor_bytes in response.tensors:
                #         logger.info(f"replicate_p2p tensor_bytes {tensor_bytes}")
                #         tensor = deserialize_torch_tensor(tensor_bytes)
    ...
    def get_current_epoch_tensor(self) -> Optional[torch.Tensor]:
        self._current_consensus_tensor = None
        self._current_consensus_tensor_expiration = None
        self.dht.get(b"key", latest=True)
    ...
    async def rpc_inference_stream(
        ...
        if not caller_peer_id.__eq__(self.peer_id):
            ...
            current_tensor = self.get_current_epoch_tensor()
            if tensor == current_tensor:
                raise ValueError("Tensor must not match the current validation tensor.")

        # TODO: Check if tensor equal this epochs current consensus tensor and don't run if so
