...
class Hoster:
    ...
    async def run(self):
        ...
        """
        # If commit failed ⸺ Return, nothing to reveal
        if self.last_inference is None:
            return

        """
        Step 2: Reveal the commit in the reveal phase
        """
        while not self.stop.is_set() and result is not None:
    ...
    async def try_load_tensor(self, key: bytes, epoch: int) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            """
            validator = self.hypertensor.get_formatted_elected_validator_node(self.subnet_id, epoch)
            print("try_load_tensor validator", validator)
    ...
    def commit(self, epoch: int, result: torch.Tensor) -> Optional[bytes]:
        """
        Commits a hashed representation of the hoster's inference result to the DHT for a given epoch.

        This function generates a salted hash of the inference `result` using `self.model_commit_fn`,
        stores the salt and result locally for later reveal, and publishes the hash (commit) to the DHT
        under a hoster-specific key for the given `epoch`.

        Args:
            epoch (int): The current epoch number for which the commit is being made.
            result (torch.Tensor): The inference result tensor to be hashed and committed.

        Returns:
            Optional[bytes]: The committed hash digest if the store operation succeeds; otherwise, None.

        Notes:
            - The commit is stored in the DHT with an expiration time defined by `_max_hoster_commit_time`.
            - The digest is stored under a namespaced key using `get_hoster_commit_key(epoch)`.
            - The record is signed using a subkey derived from the hoster's validator identity.
            - This function must be followed by a corresponding reveal phase using the stored salt and result.
        """
    ...
    def reveal(self, epoch: int, result: torch.Tensor) -> bool:
        buffer = io.BytesIO()
