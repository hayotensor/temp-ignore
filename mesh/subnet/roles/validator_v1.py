...
from mesh.subnet.utils.consensus import ConsensusScores, HosterResult
...
from mesh.substrate.chain_functions import get_epoch_length
from mesh.substrate.config import SubstrateConfigCustom
...
class Validator:
    ...
    Args:
        ...
        substrate (SubstrateConfigCustom): Interface to the blockchain network for epoch/block queries.
    """
    ...
    # def score_validators(self) -> Dict[int, float]:
    #     validator_reveal_key = get_validator_reveal_key(self.current_epoch - 1)
    #     validator_commit_key = get_validator_commit_key(self.current_epoch)
    ...
    #         logger.warning(f"[Validator] No reveals found for epoch {self.current_epoch}")
    #         return
    ...
    #     # Step 1: Gather scores per peer_id
    ...
    #     # Step 2: Compute mean per peer_id
    #     peer_means = {
    #         peer_id: statistics.mean(scores)
    #         for peer_id, scores in peer_scores.items()
    #     }

    #     # Step 3: Score each validator by total squared error from mean
    #     validator_scores = {}
    #     for i, round_scores in enumerate(results):
    #         error_sum = 0.0
    #         for score_obj in round_scores:
    #             mean = peer_means[score_obj.peer_id]
    #             error = (score_obj.score - mean) ** 2  # Squared error
    #             error_sum += error
    #         validator_scores[i] = error_sum

    #     self.consensus.store_validator_scores(validator_scores)

    #     return validator_scores

    def score_validators(self) -> Dict[str, float]:
        """
        ...
        print("score_validators results", results)

        # Step 1: Gather scores per hoster peer_id
        ...
        print("score_validators validator_scores", validator_scores)
        self.consensus.store_validator_scores(validator_scores)
    ...
    def run_hoster_validation(self, target_epoch: int):
        ...
        # 3) Score all hoster outputs
        scores = self.score_hoster_accuracy(results)
        logger.info(f"[Validator] Scores for epoch {target_epoch}: {scores}")

        # 4) Store our scores to the consensus class to be used for consensus (validating or attesting)
        self.consensus.store_hoster_scores(scores)

        # 5) Commit our validator‐score hash (under current epoch, to reveal next epoch)
        # Skips commit-reveal if hoster
        self.commit(scores, target_epoch)

    def score_hoster_accuracy(self, results: List[HosterResult]) -> Dict[str, float]:
        """
