...
class ValueWithExpiration(Generic[ValueType]):
    ...
    created_at: DHTCreatedAt

    def __iter__(self):
        return iter((self.value, self.expiration_time, self.created_at))
    ...
    def __eq__(self, item):
        if isinstance(item, ValueWithExpiration):
            # We don't check created_at time here
            return self.value == item.value and self.expiration_time == item.expiration_time
        elif isinstance(item, tuple):
            return tuple.__eq__((self.value, self.expiration_time, self.created_at), item)
        else:
            return False
...
class TimedStorage(Generic[KeyType, ValueType]):
    ...
    def __init__(self, maxsize: Optional[int] = None):
        ...
        self.data: Dict[KeyType, ValueWithExpiration[ValueType, ValueType]] = dict()
    ...
    def store(self, key: KeyType, value: ValueType, expiration_time: DHTExpiration) -> bool:
        ...
        if key in self.data:
            if self.data[key].expiration_time < expiration_time:
                self.data[key] = ValueWithExpiration(value, expiration_time, get_dht_time())
                return True
            return False
        self.data[key] = ValueWithExpiration(value, expiration_time, get_dht_time())
