...
from test_utils.dht_swarms import (
    ...
    launch_star_shaped_swarm,
    launch_star_shaped_swarms_with_record_validators,
    launch_swarm_in_separate_processes,
)
...
# pytest tests/test_dht_node_v2.py::test_launch_dht_node -rP

@pytest.mark.forked
@pytest.mark.asyncio
# @pytest.mark.xfail(reason="Flaky test", strict=False)
async def test_launch_dht_node(
    ...
    processes, dht, swarm_maddrs = launch_swarm_in_separate_processes(
        n_peers=n_peers,
        n_sequential_peers=n_sequential_peers,
...
# pytest tests/test_dht_node_v2.py::test_dht_node -rP
...
# pytest tests/test_dht_node_v2.py::test_dhtnode_reuse_get -rP
...
# pytest tests/test_dht_node_v2.py::test_dhtnode_reuse_get_with_validators -rP

@pytest.mark.forked
@pytest.mark.asyncio
async def test_dhtnode_reuse_get_with_validators():
    peers_len = 10
    ...
    peers = await launch_star_shaped_swarms_with_record_validators(record_validators=record_validators, parallel_rpc=256)
    ...
    # keys = ["k1", "k2"]
    await asyncio.gather(
        peers[peer1].store("k1", 123, mesh.get_dht_time() + 999, subkey=record_validators[peer1].local_public_key),
        peers[peer2].store("k2", 567, mesh.get_dht_time() + 999, subkey=record_validators[peer2].local_public_key),
    )

    you = random.choice(peers)

    futures1 = await you.get_many(["k1", "k2"], return_futures=True)
    store1 = you.pending_get_requests[DHTID.generate("k1")]
    store2 = you.pending_get_requests[DHTID.generate("k2")]
    print("store1", store1)
    print("store2", store2)
    assert len(store1) == 1
    assert len(store2) == 1

    results1 = await you.get_many(["k1", "k2"])
    ...
    # response = await you.get_many(["k1", "k2"])
    # for key, value in zip(keys, values):
    #     assert key in response and response[key][0] == value
...
# pytest tests/test_dht_node_v2.py::test_dht_reuse_get_with_validators -rP
...
async def test_dht_reuse_get_with_validators():
    ...
    # peers = await launch_star_shaped_swarms_with_record_validators(record_validators=record_validators, parallel_rpc=256)
    dhts = launch_dht_instances_with_record_validators(record_validators=record_validators)
    ...
    keys = ["k1", "k2"]
    values = [123, 567]
    dhts[peer1].store(keys[0], values[0], mesh.get_dht_time() + 999, subkey=record_validators[peer1].local_public_key),
    dhts[peer2].store(keys[1], values[1], mesh.get_dht_time() + 999, subkey=record_validators[peer2].local_public_key),
    ...
    # print("results2", next(iter(results1.value[DHTID.generate(keys[0])].values())))
    # commit_data = getattr(next(iter(results1[DHTID.generate(keys[0])].value.values()), None), "value", None)
