...
# pytest tests/test_hoster_commit_reveal_rsa_v5.py -rP

class DummyInferenceProtocol:
    async def run_inference_stream(self, tensor):
        return tensor * 2  # fake inference

    async def call_inference_stream(self, peer_id, prompt, tensor):
        return tensor * 2  # fake inference
...
class MockHypertensor:
    ...
    def get_formatted_elected_validator_node(self, subnet_id: int, epoch: int):
        ...


# pytest tests/test_hoster_commit_reveal_rsa_v5.py::test_hoster_commit_and_reveal_w_validation -rP
...
# pytest tests/test_hoster_commit_reveal_rsa_v5.py::test_hoster_try_load_tensor_multi_stores -rP
...
async def test_hoster_try_load_tensor_multi_stores():
    ...
    # --- Run inference via DummyInferenceProtocol ---
    result = await hoster.inference_protocol.call_inference_stream(hoster.dht.peer_id, prompt="", tensor=loaded)
    expected_result = tensor * 2
    assert torch.allclose(result, expected_result)
...
# pytest tests/test_hoster_commit_reveal_rsa_v5.py::test_hoster_run_forever -rP
